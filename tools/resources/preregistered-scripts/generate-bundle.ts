 ...
 *
 * @param domain Domain name (must match a key in `preregistered-scripts.json`).
...
/**
 * Registry of preregistered-domain content scripts.
 *
 * Key: domain.
 * Value: array of filter ID.
 *
 * Generated at build time by {@link buildPreregisteredScriptsRegistry}.
 *
 ...
 * preregistered domain (exact match) or any of its subdomains.
 *
 * @param ruleNode Parsed rule AST node.
 * @param preregisteredDomain Apex domain to match against, e.g. `"youtube.com"`.
 *
 * @returns Whether the rule targets the preregistered domain.
 */
const isRuleTargetsDomain = (
    ...
    return ruleNode.domains.children.some((domainNode) => {
        ...
        return v === preregisteredDomain || v.endsWith(`.${preregisteredDomain}`);
    });
};
...
/**
 * Builds the preregistered-scripts registry from the collected ruleset data.
 *
 ...
 *
 * @returns Registry object — flat {@link PreregisteredScriptsRegistry} mapping domain to filter ID array.
 */
const buildPreregisteredScriptsRegistry = (
    domainFilterRules: Map<string, Map<number, Set<string>>>,
 ...
 * before it is injected into preregistered-domain bundles. Warns if any
...
const scrubScriptletSource = (source: string, domain: string): string => {
    ...
    replacements.forEach(({ pattern, replacement }) => {
        if (!pattern.test(result)) {
            console.warn(`[generate-preregistered-domain-bundles] Expected pattern "${pattern}" not found in scriptlet source`);
        }
 ...
 *
 * - Private-scope (used in preregistered-domain bundles): uses an IIFE-local
        ...
        for (const [scriptletName, argsSet] of scriptletMap) {
            ...
            for (const argsJson of argsSet) {
                ...
                if (minifiedInv.code) {
                    ...
                    console.warn(`[generate-preregistered-domain-bundles] Failed to minify scriptlet invocation: ${scriptletName}`);
                }
            }
        }
    }
...
/**
 * Serializes and writes the preregistered-scripts registry as an ES module.
 *
 * Validates the output with the Node.js `vm` module.
 *
 * @param registry Registry object returned by {@link buildPreregisteredScriptsRegistry}.
 * @param outputPath Absolute path for the output `.js` file.
 *
 * @throws If the file fails syntax validation.
 */
const writePreregisteredScriptsRegistry = async (
    registry: PreregisteredScriptsRegistry,
    outputPath: string,
): Promise<void> => {
    const content = `// AUTO-GENERATED — do not edit manually. Re-run pnpm resources:mv3 to update.
export const preregisteredDomainScripts = ${JSON.stringify(registry, null, 4)};${NEWLINE_CHAR_UNIX}`;

    await fs.writeFile(outputPath, content, 'utf-8');
};

/**
 * Generates static JS bundles for each preregistered domain from the pre-built MV3
 * filter lists.
 *
...
export const generatePreregisteredDomainBundles = async (
    ...
    const domainFilterRules = new Map<string, Map<number, Set<string>>>(
        preregisteredDomains.map((d) => [d, new Map()]),
    );
        ...
        filterListNode.children.forEach((ruleNode) => {
            ...
            if (isScriptletRule(ruleNode)) {
                try {
                    ...
                    console.warn(`[generate-preregistered-domain-bundles] Skipping invalid scriptlet rule; Error: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
        });
    }
    ...
    for (const [domain, filterIds] of allDomainIds) {
        ...
        for (const filterId of filterIds) {
            ...
            console.log(`[generate-preregistered-domain-bundles] ${domain}-${filterId}: ${totalRuleCount} unique rules (${jsRules?.size ?? 0} JS + ${scriptletCount} scriptlets)`);
            ...
            if (!bundleContent) {
                console.warn(`[generate-preregistered-domain-bundles] No compilable rules for ${domain}-${filterId}, skipping bundle.`);
                // eslint-disable-next-line no-continue
                continue;
            }
            ...
            console.log(`[generate-preregistered-domain-bundles] Wrote ${fileName} (${sizeKb} KB)`);
        }
    }

    // Build and write the preregistered-scripts registry
    const registry = buildPreregisteredScriptsRegistry(
        domainFilterRules,
        domainFilterScriptlets,
    );

    const registryPath = path.join(outputDir, 'registry.js');
    await writePreregisteredScriptsRegistry(registry, registryPath);

    const domainCount = Object.keys(registry).length;
    console.log(`[generate-preregistered-domain-bundles] Wrote registry.js with ${domainCount} domain(s)`);
};
