...
describe('PagesApi', () => {
    ...
    it('getIssueReportUrl', async () => {
        ...
        expect(reportUrl).toContain('?app=browser_extension&');
        expect(reportUrl).toContain(`&from=${reportedFrom}&`);
        expect(reportUrl).toContain(`&url=${encodeURIComponent(websiteUrl)}&`);
        expect(reportUrl).toContain('&product_type=Ext&');
    });
});
