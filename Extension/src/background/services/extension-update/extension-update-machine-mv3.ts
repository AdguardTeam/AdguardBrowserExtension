...
type EventType = {
    ...
    /**
     * Whether the extension update failed after a reload (version unchanged).
     */
    isUpdateFailedAfterReload?: boolean;
};
    ...
    states: {
        [ExtensionUpdateFSMState.Idle]: {
            on: {
                 ...
                 * - isUpdateFailedAfterReload: Extension reloaded but update failed → Failed;
                ...
                [ExtensionUpdateFSMEvent.Init]: [
                    ...
                    {
                        guard: ({ event }: { event: EventType }): boolean => !!event.isUpdateFailedAfterReload,
