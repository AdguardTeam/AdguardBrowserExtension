...
import { type ManualExtensionUpdateData, ManualExtensionUpdateDataValidator } from './types';
 ...
 *
 * FIXME: Explain in docs why browserStorage is used here, while AutoUpdateHandler uses UpdateStateManager.
 */
export class ManualUpdateHandler {
    ...
    /**
     * Callback when update application completes.
     */
    private readonly onUpdateApplyComplete: () => void;
     ...
     * @param options.onUpdateApplyComplete Callback when update application completes.
    ...
    constructor(options: {
        ...
        this.onUpdateApplyComplete = options.onUpdateApplyComplete;
    ...
    public async check(): Promise<void> {
        ...
        this.stateManager.isManualCheck = true;
        ...
        this.stateManager.isManualCheck = false;
    }

    /**
     * Clears the checking update timeout if it was set.
     */
    public clearCheckTimeout(): void {
        // eslint-disable-next-line no-restricted-globals
    ...
    public static async handleReload(isUpdate: boolean): Promise<void> {
        ...
        if (!manualExtensionUpdateData) {
            logger.debug('[ext.ManualUpdateHandler.handleReload]: No manual extension update data found');
            return;
        }
         ...
         */
        if (!isUpdate) {
            const manualExtensionUpdateData: ManualExtensionUpdateData = {
                initVersion,
     ...
     */
    public static async getUpdateData(): Promise<ManualExtensionUpdateData | null> {
        const manualExtensionUpdateData = await ManualUpdateHandler.retrieveUpdateData();
     ...
     * @returns Manual extension update data or null if not found.
     */
    private static async retrieveUpdateData(): Promise<ManualExtensionUpdateData | null> {
        try {
