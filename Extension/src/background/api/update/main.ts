...
import { FilterListPreprocessor } from './assets/preprocessor/preprocessor';
...
export class UpdateApi {
    ...
    private static async migrateFromV13toV14(): Promise<void> {
         ...
         *
         * OLD FORMAT (custom preprocessor):
         * - `rawFilterList_<id>`: Preprocessed filter list with converted rules
         * - `conversionMap_<id>`: Map of converted rules to their original text
         * - `filterList_<id>`: Binary serialized filter data (unused, gets deleted)
         * - `sourceMap_<id>`: Source map for binary data (unused, gets deleted).
         *
         * NEW FORMAT (tsurlfilter FilterList):
         * - `filterContent_<id>`: Converted filter list text
         * - `conversionData_<id>`: Conversion metadata for restoring original rules.
         *
         * Migration steps:
         * 1. Restore original filter text from preprocessed data (rawFilterList + conversionMap)
         * 2. Re-process with new FilterList class (which handles conversion internally)
         * 3. Save in new format and clean up old keys.
         */
        ...
        for (const key of keys) {
            if (key.startsWith('filterList_') || key.startsWith('sourceMap_')) {
                // Remove binary serialized data (filterList_* and sourceMap_*).
                // These were generated by the old preprocessor but never actually used,
                // as we always relied on rawFilterList + conversionMap for filter data.
                keysToRemove.push(key);
            }

            if (key.startsWith('rawFilterList_') || key.startsWith('conversionMap_')) {
                // Extract filter ID from keys like "rawFilterList_123" or "conversionMap_123"
                ...
                if (!Number.isNaN(id)) {
                    ...
                    // does not happen during normal operation
                    logger.error(`[ext.UpdateApi.migrateFromV13toV14]: Invalid filter ID: ${rawId}`);
                }

                // Mark old format keys for removal after successful migration
                keysToRemove.push(key);
            }
        }

        // Migrate all filters in parallel for better performance
        await Promise.all(
            Array.from(filterIds).map(
                async (id: number): Promise<void> => {
                    const [rawFilterList, conversionMap] = await Promise.all([
                        hybridStorage.get(`rawFilterList_${id}`),
                        hybridStorage.get(`conversionMap_${id}`),
                    ]);

                    // Step 1: Restore the original filter list text from the old preprocessed format.
                    // This is necessary because FilterList needs to process the original rules,
                    // not the already-converted ones stored in rawFilterList.
                    const originalFilterList = FilterListPreprocessor.getOriginalFilterListText({
                        rawFilterList: rawFilterList as string,
                        conversionMap: conversionMap as Record<string, string>,
                    });
                    ...
                    // to maintain independence from potential future FiltersStorage API changes.
                    await hybridStorage.set(`filterContent_${id}`, convertedFilterList.getContent());
                    await hybridStorage.set(`conversionData_${id}`, convertedFilterList.getConversionData());
                },
            ),
        );

        // Clean up all old format keys after successful migration
