QUnit.test("Rules Constructor for Assistant", function(assert) {
    ...
    var ruleText = DevToolsRulesConstructorHelper.constructRuleText(element, options);
    assert.equal(ruleText, 'example.org###test-div');

    options.ruleType = 'URL';
    ruleText = DevToolsRulesConstructorHelper.constructRuleText(element, options);
    ...
    options.cssSelectorType = 'STRICT_FULL';
    options.isBlockOneDomain = true;
    ruleText = DevToolsRulesConstructorHelper.constructRuleText(elementHref, options);
...
QUnit.test("Rules Constructor for special elements", function(assert) {
    ...
    var ruleText = DevToolsRulesConstructorHelper.constructRuleText(elementHref, options);
    ...
    ruleText = DevToolsRulesConstructorHelper.constructRuleText(elementDivId, options);
...
QUnit.test("Rules Constructor for CSS selector", function(assert) {
    var selector;
    selector = DevToolsRulesConstructorHelper.constructRuleCssSelector('lenta.ru##div.test-class-two#test-id-div$domain=example.org');
    assert.equal('div.test-class-two#test-id-div', selector);

    selector = DevToolsRulesConstructorHelper.constructRuleCssSelector('lenta.ru###test-div > h2:last-child');
    assert.equal('#test-div > h2:last-child', selector);

    selector = DevToolsRulesConstructorHelper.constructRuleCssSelector('##div#test-id-div[title="Share on Twitter"]');
    assert.equal('div#test-id-div[title="Share on Twitter"]', selector);

    selector = DevToolsRulesConstructorHelper.constructRuleCssSelector('http://test.com/page$domain=example.org');
    assert.equal(selector, "[src*=\"http://test.com/page\"]");

    selector = DevToolsRulesConstructorHelper.constructRuleCssSelector('||http://rutorads.com^$popup');
    assert.equal(selector, "[src*=\"http://rutorads.com\"]");

    selector = DevToolsRulesConstructorHelper.constructRuleCssSelector("#%#window.AG_onLoad = function(func) { if (window.addEventListener) { window.addEventListener('DOMContentLoaded', func); } };");
...
QUnit.test("Dot Classes", function(assert) {
    ...
    options.cssSelectorType = 'SIMILAR';
    var selector = DevToolsRulesConstructorHelper.constructRuleText(element, options);
