...
UrlFilter.prototype = {
	 ...
	 */
	isFiltered: function (url, referrerHost, requestType, thirdParty, skipGenericRules) {
		// First looking for the rule marked with $important modifier
		var rule = this.importantRulesTable.findRule(url, referrerHost, thirdParty, requestType, !skipGenericRules);

		if (rule == null) {
			rule = this.basicRulesTable.findRule(url, referrerHost, thirdParty, requestType, !skipGenericRules);
		}
		return rule;
	},
