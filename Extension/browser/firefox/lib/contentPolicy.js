...
var WebRequestImpl = exports.WebRequestImpl = {
	...
	_shouldBlockRequest: function (tab, requestUrl, requestType, node, contentType) {
		...
		this._processCollapseParentElement(tab, tabUrl, node);
	...
	_filterSafebrowsing: function (requestUrl, tab) {
		...
		this.antiBannerService.getRequestFilter().checkSafebrowsingFilter(requestUrl, function (safebrowsingUrl) {
			var xulTab = tabUtils.getTabForId(tab.id);
			if (xulTab) {
	...
	_processCollapseParentElement: function (tab, tabUrl, node) {

		if (!node) {
			return;
		}

		//try to collapse parent <a> element
		var parent = node.parentNode;
		if (!(parent && parent.localName == "a" && parent.href)) {
			return;
		}

		var contentType = WebRequestHelper.contentTypes.TYPE_OTHER;
		var requestType = WebRequestHelper.getRequestType(contentType);
		var requestUrl = WebRequestHelper.getUrlAscii(parent.href);

		var requestEvent = this.webRequestService.processRequest(tab, requestUrl, tabUrl, requestType);
		if (requestEvent.requestBlocked) {
			this._collapseElement(parent, contentType);
			this.webRequestService.postProcessRequest(requestEvent);
		}
	},
