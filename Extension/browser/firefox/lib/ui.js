...
var UI = exports.UI = {
	...
	openCurrentTabFilteringLog: function () {
		var tabInfo = this.filteringLog.getTabInfo(tabs.activeTab);
	...
	openAssistant: function (assistantOptions) {
		...
		var addUserRule = function (message) {
			if (this.framesMap.isTabAdguardDetected(tabs.activeTab)) {
		...
		activeWorker = activeTab.attach({
			...
			contentScriptFile: [
				...
				self.data.url("content/content-script/postload.js"),
	...
	removeCurrentTabFromAdguardWhiteList: function () {
		var rule = this.framesMap.getTabAdguardUserWhiteListRule(tabs.activeTab);
		if (rule) {
	...
	_initEventListener: function () {
		...
		EventNotifier.addListener(function (event, tab, reset) {
			...
			if (Prefs.mobile || tab.id != tabs.activeTab.id) {
				return;
			}
		...
		sdkWindows.on('activate', function () {
			updateActiveTabIcon(tabs.activeTab);
		});
	},
	...
	_updatePopupButtonState: function (tab, reloadFrameData) {
		...
		if (!tab || tab.id != tabs.activeTab.id) {
			return;
		}
