...
var PopupButton = {
	...
	init: function (UI, SdkButton) {
		...
		adguard.windows.getAll(function (wins, nativeWins) {
			for (var i = 0; i < nativeWins.length; i++) {
				this.applyToWindow(nativeWins[i]);
			}
		}.bind(this));
	...
	addToolbarButton: function () {
		...
		var options = {
			...
			onBuild: function (doc) {
				...
				var toolbarItem = doc.createElementNS("http://www.mozilla.org/keymaster/gatekeeper/there.is.only.xul", "toolbarbutton");
	...
	shutdown: function () {
		...
		adguard.windows.getAll(function (wins, nativeWins) {
			for (var i = 0; i < nativeWins.length; i++) {
				this.removeFromWindow(nativeWins[i]);
			}
		}.bind(this));
	}
};